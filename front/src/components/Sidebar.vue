<template>
    <aside
        :class="[
            ...
            isSidebarOpen ? 'flex' : 'inline-flex px-2',
            props.class,
        ]"
    >
        <div class="h-full bg-hollow flex flex-col pb-4">
            <Menu
                v-if="user && isSidebarOpen"
                class="w-full"
                use-squared-button
                size="small"
            >
            ...
            <nav v-if="isSidebarOpen" class="mt-2">
                <div
                    class="px-4 py-2 font-semibold bg-hollow-light flex justify-between items-center gap-2"
                >
        ...
        <Button
            is-square
            variant="hollow"
            :title="`${!isSidebarOpen ? 'Expand sidebar' : 'Close sidebar'}`"
            @click="toggleSidebar"
            :class="`mx-2 ${isSidebarOpen ? 'ml-3' : ''}`"
        >
            <ChevronDoubleRightIcon class="h-4 w-4" v-if="!isSidebarOpen" />
            <ChevronDoubleLeftIcon class="h-4 w-4" v-if="isSidebarOpen" />
        </Button>
    </aside>
</template>
